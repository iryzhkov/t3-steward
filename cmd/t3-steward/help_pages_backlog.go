...
func backlogHelpPages() []helpPage {
	...
	pages = append(pages,
		...
		backlogReadPage("backlog projects", "the configured projects and, per project, the eligible workers and the routes they advertise.",
			...
			[]helpFlag{{Name: "--project", Value: "NAME", Default: "every project", Text: "Report one project only. Unfiltered the answer is the whole catalog, tens of kilobytes on a real fleet; naming the project is the difference between that and a couple of kilobytes."}},
			[]string{"projects"}, backlogReadSites("projects"),
			"Read-only. \"t3-steward task run\" derives its project and route from this answer, and \"t3-steward models\" reports the same routes joined with their quota state."),
