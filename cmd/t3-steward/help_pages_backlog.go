...
func backlogHelpPages() []helpPage {
	...
	pages = append(pages,
		...
		backlogReadPage("backlog usage", "raw provider token samples joined to authoritative V2 dispatch identity.",
			"t3-steward backlog usage [<workflow-run>] [--json]",
			nil, []string{"usage", "usageSemantics"}, backlogReadSites("usage"),
			"Read-only and report-only. With no run it includes explicit unattributed samples. Call rows and whole-turn rows overlap and must not be summed."),
