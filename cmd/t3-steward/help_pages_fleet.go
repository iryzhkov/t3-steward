	...
	pages := []helpPage{
		{
			...
			Flags: []helpFlag{
				...
				{Name: "--github", Value: "run <id>|pr <n>", Default: "none", Text: "GitHub condition. The id may be qualified as owner/name#<n> or given as the target's URL (https://github.com/owner/name/pull/<n>, https://github.com/owner/name/actions/runs/<id>), which also sets --repo; a URL alone names its kind. Any other pr id is passed to gh as a branch name. GitHub Enterprise hosts are not supported. The target is read once at registration with gh and fixed arguments; unreadable, already met or already failed is refused."},
				{Name: "--repo", Value: "owner/name", Default: "the repository the target names, else the repository of the current directory, resolved and recorded at registration", Text: "The repository of a github target. One that disagrees with the repository the target names is refused."},
				...
				{Name: "--attention", Value: "approval|direction", Default: "none", Text: "Task-bound request for an authenticated human decision."},
				{Name: "--prompt", Value: "TEXT", Default: "none", Text: "The exact question or direction requested by --attention."},
		...
		},
		{
			...
			Flags: []helpFlag{
				{Name: "--decision-id", Value: "ID", Default: "generated", Text: "Stable decision identity for safe replay."},
				{Name: "--wait", Value: "ID", Default: "required", Text: "Attention wait ID."},
				{Name: "--request-id", Value: "ID", Default: "required", Text: "Original attention request ID."},
				{Name: "--run", Value: "ID", Default: "required", Text: "Workflow run fence."},
				{Name: "--task-id", Value: "ID", Default: "required", Text: "Task fence."},
				{Name: "--attempt", Value: "ID", Default: "required", Text: "Attempt fence."},
				{Name: "--assignment", Value: "ID", Default: "required", Text: "Assignment fence."},
				{Name: "--assignment-epoch", Value: "N", Default: "required", Text: "Assignment epoch fence."},
				{Name: "--worker", Value: "ID", Default: "required", Text: "Worker fence."},
				{Name: "--thread", Value: "ID", Default: "required", Text: "Thread fence."},
				{Name: "--revision", Value: "N", Default: "required", Text: "Coordinator-observed attempt revision fence."},
				{Name: "--content-digest", Value: "SHA256", Default: "required", Text: "Canonical attention request content fence."},
				{Name: "--deadline", Value: "RFC3339", Default: "required", Text: "Exact decision deadline fence."},
				{Name: "--decision", Value: "approve|resume|hold|stop|change", Default: "required", Text: "Decision kind."},
				{Name: "--reason", Value: "TEXT", Default: "required", Text: "Audited reason."},
				{Name: "--change", Value: "TEXT", Default: "none", Text: "Proposed change; valid only with decision change."},
			},
			Exits:    []helpExit{{0, "receipt returned"}, {3, "client configuration"}, {4, "approver authentication"}, {5, "no coordinator answered"}, {6, "timeout"}, {8, "decision rejected"}},
			JSONKeys: []string{"decision", "requestedBy", "state", "failure", "receivedAt", "appliedAt", "deliveredAt", "observedAt"},
			Notes:    "The configured credential must be an approver credential. Ordinary local, remote-admin, supervisor and worker authority cannot answer an attention request.",
			Parsers:  []parserSite{{Func: "cmdNodeWait", Case: "answer"}},
		},
		{
			Path:    "wait cancel",
