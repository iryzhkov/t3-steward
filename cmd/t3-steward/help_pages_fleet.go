...
func fleetHelpPages() []helpPage {
	pages := []helpPage{
		...
		{
			Path:    "wait list",
			Purpose: "the waits this thread, or every thread, is holding, with their kind and outcome.",
			Usage: []string{
				"t3-steward wait list [--thread ID] [--all] [--json]",
				"t3-steward wait list --native [--json]",
			},
			Flags: []helpFlag{
				{Name: "--thread", Value: "ID", Default: "the thread this process resolves to", Text: "List the local checks of one thread."},
				{Name: "--all", Default: "off", Text: "List the local checks of every thread on this host."},
				{Name: "--native", Default: "off", Text: "List the coordinator-held waits instead: node, quota and every task-bound wait, with their outcomes and delivery state."},
				jsonFlag("the list"),
			},
			Exits:    []helpExit{{0, "listed"}, {1, "the state database could not be read, or a bad option"}, {3, "--native only: client configuration"}, {5, "--native only: no coordinator answered"}, {6, "--native only: timeout"}, {8, "--native only: the coordinator refused the query"}},
			JSONKeys: []string{"id", "threadId", "kind", "status", "runs", "lastExit", "taskWaitId", "name", "reason"},
			JSONNote: "Without --native the document is an array of local checks. With --native it is the coordinator's document, with waits and taskWaits.",
			Notes:    "Without --native it reads this host's state database only.",
			Parsers:  []parserSite{{Func: "cmdWaitList"}, {Func: "cmdNodeWait", Case: "list"}},
		},
		{
			Path:    "wait cancel",
