...
func reloadUnblockAction(attempt domain.Attempt, found bool) string {
	...
	if attempt.IsSupervisionActivation() {
		...
		return fmt.Sprintf("an overseer activation assignment of run %s; the coordinator releases an unclaimed offer once its activation ends (t3-steward campaign supervision show %s)",
			attempt.WorkflowRunID, attempt.WorkflowRunID)
	}
