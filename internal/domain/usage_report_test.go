...
func TestNormalizeUsageReportAvoidsOverlapAndCumulativeInflation(t *testing.T) {
	...
	sample := func(id, provider, thread, model, kind string, cumulative, input, write, read, output int64) UsageSample {
		return UsageSample{
			...
			CacheReadTokens: read, OutputTokens: output, Attribution: attribution,
		}
	}
	...
	if got := report.Totals; got.UncachedInputTokens != 40 || got.CacheWriteTokens != 6 ||
		...
		!got.ProviderCostReported || got.NormalizedSamples != 4 || got.Calls != 2 || got.Turns != 2 {
...
func TestNormalizeUsageReportRetainsCallOnlyTurnAfterLatestSummary(t *testing.T) {
	now := time.Date(2026, 9, 22, 18, 0, 0, 0, time.UTC)
	attribution := UsageAttribution{Status: UsageAttributed, WorkerID: "worker", WorkflowRunID: "run",
		TaskID: "task", AttemptID: "attempt", AssignmentID: "assignment", Role: ExecutionRoleExecutor}
	before := UsageSample{WorkerID: "worker", ProviderInstanceID: "claude", ThreadID: "thread",
		Model: "claude-sonnet", ObservedAt: now, SourceEventID: "call-covered", Kind: UsageKindCall,
		InputTokens: 100, OutputTokens: 10, Attribution: attribution}
	turn := UsageSample{WorkerID: "worker", ProviderInstanceID: "claude", ThreadID: "thread",
		Model: "claude-sonnet", ObservedAt: now.Add(time.Minute), SourceEventID: "turn#sonnet", Kind: UsageKindTurn,
		InputTokens: 100, OutputTokens: 10, Attribution: attribution}
	after := UsageSample{WorkerID: "worker", ProviderInstanceID: "claude", ThreadID: "thread",
		Model: "claude-sonnet", ObservedAt: now.Add(2 * time.Minute), SourceEventID: "call-unmatched", Kind: UsageKindCall,
		InputTokens: 7, OutputTokens: 3, Attribution: attribution}

	report := NormalizeUsageReport(UsageReport{Samples: []UsageSample{before, after, turn}},
		UsageNormalizationContext{Now: now.Add(3 * time.Minute), RunProgress: ProgressActive})
	if report.Totals.UncachedInputTokens != 107 || report.Totals.OutputTokens != 13 ||
		report.Totals.NormalizedSamples != 2 || report.Coverage.ExcludedOverlapCount != 1 ||
		report.Coverage.UnmatchedCallCount != 1 || report.Coverage.State != UsageCoveragePartial {
		t.Fatalf("mixed summary report = %#v", report)
	}
}
