...
func NormalizeUsageReport(report UsageReport, context UsageNormalizationContext) UsageReport {
	...
	// Only the unbound samples that could be this run's own make its coverage
	// partial; the rest of the fleet's unbound samples in the same window are
	// reported beside it as context. See sqlite.Store.AttributedUsage for how
