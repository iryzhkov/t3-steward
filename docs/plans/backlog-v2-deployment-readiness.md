...
Normandy self-SSH uses its existing key constrained to an exact forced-command wrapper for control, the two artifact operations, and read-only `git-upload-pack` for the Citadel repository. `git ls-remote` returned the exact integrated Citadel head and a command naming another repository was rejected with exit 64. `NoNewPrivileges` remains enabled. `ProtectSystem=full` was disabled because a user-service mount namespace maps root-owned `/etc/ssh` client configuration to uid 65534, which OpenSSH rejects; the failure was independently reproduced with `systemd-run --user`.
