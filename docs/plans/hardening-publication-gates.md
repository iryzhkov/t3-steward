...
## Gate 4b: agent-facing skills and instructions

The CLI is an API for agents, and the skills are how agents learn it. A published binary whose
skills still teach the old shapes is a half-published change, so this gate sits before the fleet
sees the new behaviour.

Ownership, verified 2026-09-14:

- `t3-backlog` and `t3-wait` have their source in omarchy-setup at `config/claude/skills/`.
- `t3-campaign` has **no source repository**. It exists only as `~/.claude/skills/t3-campaign/SKILL.md`
  on this host and reaches the fleet only because UpKeeper's `agent-environment` component
  captures the controller's file. That is the same unowned-live-state pattern this whole effort
  exists to remove, so the fix is to give it a source in omarchy-setup alongside the other two.
- `~/.config/agents/t3-steward.md` is generated by omarchy-setup's `config/bin/agents-instructions-gen`
  and carries all three skills for Codex and OpenCode. It is a derived file: edit the sources,
  regenerate, never hand-edit.

What the skills must teach after this work:

1. `campaign check` as a distinct verb, what `ready`, `accepted_waiting` and `impossible` mean for
   an agent about to end its turn, and that `submit` checks by default.
2. That `--allow-unverified` is operator-oriented and agents should not use it.
3. Submitting from a non-coordinator host through the configured coordinator client, and
   `coordinator identity` to prove which coordinator will answer. No skill, example or error may
   teach `ssh <coordinator> t3-steward ...`.
4. The transport exit-code classes, so an agent can branch on authentication versus unavailable
   versus rejected.
5. Task-bound waits: `wait add --task current` parks the turn, so the turn ends there; the task is
   not complete, not verified and not failed. Interactive waits stay what they were, and the two
   are distinguished by what they wake and what they can change.
6. `each` versus `all`, and what a failed or timed-out wait delivers on resume.
7. `campaign rerun --from` and `--notify-thread current`.

The gate passes when the sources are updated, the derived file is regenerated from them, and a
read of each skill by a fresh agent produces the correct shortest path without mentioning a remote
shell.
