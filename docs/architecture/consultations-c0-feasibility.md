...
The T3 source is not present in the inspected workspace, so support below its HTTP dispatch API could not be verified. The exposed API is nevertheless insufficient for Steward to request or prove these controls. This is a C0 blocker for strict mode and should be raised to the T3 adapter owner before consultation schema implementation proceeds.

A weaker pilot may use: fresh thread, immutable byte-bounded prompt/attachments, no optional source/web reads, minimal tools, four-turn prompt instruction, provider output option where supported, and retrospective usage checks. Its contract must say “trusted-agent bounded-input pilot”; it must not use “hard context bound,” qualify section 15, or enable project defaults.
